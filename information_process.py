...
def calc_entropy_for_specipic_t(current_ts, px_i):
    ...
    # current_ts = np.asarray(current_ts, order='C')
    b2 = np.ascontiguousarray(current_ts).view(np.dtype((np.void, current_ts.dtype.itemsize * current_ts.shape[1])))
    unique_array, unique_inverse_t, unique_counts = \
        np.unique(b2, return_index=False, return_inverse=True, return_counts=True, axis=1)
...
def extract_probs(label, x):
    ...
    unique_array, unique_indices, unique_inverse_x, unique_counts = \
        np.unique(b, return_index=True, return_inverse=True, return_counts=True, axis=1)
    unique_a = x[unique_indices]
    
    # unique_a = np.asarray(unique_a, order='C')
    ...
    unique_array_y, unique_indices_y, unique_inverse_y, unique_counts_y = \
        np.unique(b_y, return_index=True, return_inverse=True, return_counts=True, axis=1)
...
def calc_information_sampling(data, bins, pys1, pxs, label, b, b1, len_unique_a, p_YgX, unique_inverse_x,
    ...
    # digitized = np.asarray(digitized, order='C')
